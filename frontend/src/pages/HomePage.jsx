...
const GeneratedResult = ({ result }) => {
  ...
  if (result.error) return <div className="generated-error">{result.error}</div>
  if (result.flashcards) return (
    <div className="generated-content flashcard-results">
      <div className="result-heading"><strong>{result.count} flashcards</strong><span>Click a card to reveal the answer</span></div>
      <div className="flashcard-grid">{result.flashcards.map((card, index) => <button className={`flashcard ${flippedCards[index] ? 'flipped' : ''}`} type="button" key={`${card.front}-${index}`} onClick={() => setFlippedCards({ ...flippedCards, [index]: !flippedCards[index] })}><span className="flashcard-label">{flippedCards[index] ? 'Answer' : 'Prompt'}</span><strong>{flippedCards[index] ? card.back : card.front}</strong><small>{flippedCards[index] ? 'Click to see prompt' : 'Click to reveal answer'}</small></button>)}</div>
    </div>
  )
  if (result.quiz) return (
    <div className="generated-content quiz-results">
      <div className="result-heading"><strong>{result.total_questions} question quiz</strong><span>Test your understanding</span></div>
      {result.quiz.map((item, index) => <article className="quiz-question" key={`${item.question}-${index}`}><span className="quiz-number">0{index + 1}</span><h3>{item.question}</h3><div className="quiz-options">{item.options.map((option, optionIndex) => <button className={answers[index] === optionIndex ? 'chosen' : ''} type="button" key={option} onClick={() => setAnswers({ ...answers, [index]: optionIndex })}>{String.fromCharCode(65 + optionIndex)}. {option}</button>)}</div>{answers[index] !== undefined && <p className={answers[index] === item.correct_option_index ? 'answer-correct' : 'answer-wrong'}>{answers[index] === item.correct_option_index ? 'Correct answer' : `Not quite. The answer is ${String.fromCharCode(65 + item.correct_option_index)}.`}</p>}</article>)}
    </div>
  )
  return <div className="generated-content text-result"><div className="result-heading"><strong>{result.summary ? 'Summary' : 'Study notes'}</strong><span>Generated from your PDF</span></div><div className="generated-text">{result.summary || result.study_notes}</div></div>
}
